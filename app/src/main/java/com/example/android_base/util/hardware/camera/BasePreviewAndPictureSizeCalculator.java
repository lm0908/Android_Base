...
package com.example.android_base.util.hardware.camera;
...
public class BasePreviewAndPictureSizeCalculator {
	...
	public Size[] getPreviewAndPictureSize(){
		...
		try{
			...
			/* 首先获取预览尺寸集合和输出图片尺寸集合 */
			List<BestPreviewSizeCalculator.CameraSize> previewCameraSizes = new ArrayList<BestPreviewSizeCalculator.CameraSize>(supportPreviewSizes.size());
			for(Size size2 : supportPreviewSizes){
				previewCameraSizes.add(new BestPreviewSizeCalculator.CameraSize(size2));
			}
			List<BestPreviewSizeCalculator.CameraSize> pictureCameraSizes = new ArrayList<BestPreviewSizeCalculator.CameraSize>(supportPictureSizes.size());
			for(Size size : supportPictureSizes){
				pictureCameraSizes.add(new BestPreviewSizeCalculator.CameraSize(size));
			}
